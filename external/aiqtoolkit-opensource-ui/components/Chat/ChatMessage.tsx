...
export const ChatMessage: FC<Props> = memo(({ message, messageIndex, onEdit}) => {
  console.log('aiq - ChatMessage rendering:', {
    role: message?.role,
    content: message?.content,
    hasContent: !!message?.content,
    contentLength: message?.content?.length,
    hasIntermediateSteps: !!message?.intermediateSteps?.length
  });
  ...
  if (!message || ((!message.content || message.content.trim() === '') 
    ...
    console.log('aiq - ChatMessage returning null');
    return null;
  }
  ...
  const prepareContent = ({ 
    ...
    // If result is empty, return a space to prevent rendering issues
          ...
          {message.role === 'user' ? (
            ...
            <div className="flex flex-col w-[90%]">
              <div className="flex flex-col gap-2">    
                ...
                {/* Add consensus button for assistant messages */}
                {message.role === 'assistant' && message.consensusId && (
                  <ConsensusButton 
