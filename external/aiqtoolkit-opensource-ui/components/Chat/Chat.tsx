...
export const Chat = () => {
  ...
  let websocketLoadingToastId: string | undefined = undefined;
  ...
  const connectWebSocket = async (retryCount = 0) => {
    ...
    return new Promise((resolve) => {
      const ws = new WebSocket((sessionStorage.getItem('webSocketURL') || webSocketURL || 'ws://localhost:8000/v1/ws'));
    ...
    async (message: Message, deleteCount = 0, retry = false) => {
      ...
      if (selectedConversation) {
        ...
        try {
          ...
          // Check if this is a streaming endpoint by looking at the URL
          const actualChatCompletionURL = sessionStorage.getItem('chatCompletionURL') || chatCompletionURL;
          const isStreaming = actualChatCompletionURL.includes('stream');
          ...
          if (isStreaming) {
            ...
            homeDispatch({ field: 'loading', value: false });
            console.log('aiq - Starting streaming response');
            ...
            console.log('aiq - received response text:', responseText);
            console.log('aiq - existing messages:', updatedConversation.messages.length);
            
            // Try to parse JSON to extract the actual message content
            let messageContent = responseText;
            try {
              const parsedResponse = JSON.parse(responseText);
              // Extract the actual message from the response
              messageContent = parsedResponse.output || parsedResponse.answer || parsedResponse.value || parsedResponse.content || responseText;
            } catch (e) {
              // If parsing fails, use the raw response text
              console.log('aiq - using raw response text (JSON parse failed)');
            }
            ...
            console.log('aiq - new messages:', updatedMessages.length);
            console.log('aiq - last message content:', updatedMessages[updatedMessages.length - 1].content);
            updatedConversation = {
              ...updatedConversation,
              messages: updatedMessages,
            };
